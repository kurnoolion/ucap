...
- `ERROR_CODES` — value — pub — Central registry of all error codes (12 entries at v1).
- `ErrorCode` — class — pub — A stable, registered error code per D-009.
- `ErrorSeverity` — class — pub — Severity enum (`E` or `W`).
- `PREFIX_REGISTRY` — value — pub — Three-letter prefix → module-name mapping (5 entries at v1).
- `QCField` — class — pub — One field declaration in a `QCTemplate`.
- `QCTemplate` — class — pub — Fixed-field schema for `ReportRecord` validation per `(module_prefix, artifact_type)`.
- `QC_REGISTRY` — value — pub — Pre-registered `QCTemplate` dict; key `"<module-name>:<artifact-type>"`.
- `REDACTION_CATEGORIES` — value — pub — The six placeholder categories: `DEV`, `FW`, `OP`, `ID`, `PATH`, `SESS`.
- `Redactor` — class — pub — Forward-substitution layer applied at report-emit time (D-012).
- `ReportRecord` — class — pub — One compact report record — RPT, MET, or QC (D-009).
- `ReportType` — class — pub — Type tag enum: `RPT`, `MET`, `QC`.
- `ReportWriter` — class — pub — Buffers `ReportRecord` instances and flushes serialized lines.
- `_format_value` — function — internal — Serialize a field value to its on-the-wire form per D-013 grammar.
- `_parse_value` — function — internal — Parse a field value from its on-the-wire form into a Python type.
- `_validate_registries` — function — internal — Foreign-key check at module load (every `ERROR_CODES` prefix is in `PREFIX_REGISTRY`).
- `format_code` — function — pub — Format a registered code's message template (thin `str.format` wrapper; see Invariants).
- `get_code` — function — pub — Look up a registered error code; raises `KeyError` with the DGN-E002 template on miss.
<!-- END:STRUCTURE -->
